"""Python port of the dataviz skill's validate_palette.js categorical checks.

Written because this machine has no Node runtime. Thresholds, the Machado
severity-1.0 CVD matrices, and the OKLab math are transcribed from
scripts/validate_palette.js so results are comparable to the reference tool.

usage:
  python validate_palette.py "#hex,#hex,..." [--mode light|dark]
...
def validate(palette, mode="light", surface=None, pairs="adjacent"):
    ...
    rows.append(("Contrast vs surface", "WARN" if low else "PASS",
                 f"below {CONTRAST_MIN}:1 - relief required (visible labels or "
                 f"table view): {low}" if low else f"all {len(palette)} >= {CONTRAST_MIN}:1"))
...
def main():
    ...
    rows, ok = validate(palette, args.mode, surface, args.pairs)

    print(f"\nPalette ({args.mode}, surface {surface}, {args.pairs}): {len(palette)} slots")
